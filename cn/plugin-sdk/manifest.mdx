---
title: "autClaw 插件 manifest 声明头"
icon: "file-code"
description: "在 autClaw 插件文件顶部声明 author、runtime、rule、param、dependency、router 和 cron 等 manifest headers。"
keywords: ["插件 manifest", "manifest headers", "rule", "param", "cron"]
---

Manifest headers 是插件文件顶部的注释声明。autClaw 读取这些声明来显示插件信息、匹配消息、生成配置表单、安装依赖和注册路由。

<Note>
  普通插件作者只需要关注本页字段。连接新 IM 平台时，请继续看 [`适配器插件开发规范`](/cn/plugin-sdk/adapter-development)。
</Note>
...
这些头注主要给 `plugin/adapters/` 下的适配器 worker 使用。完整接入流程见 [`适配器插件开发规范`](/cn/plugin-sdk/adapter-development)。

| Header | 示例 | 作用 |
|---|---|---|
| `im_type` | `qq` | 渠道稳定标识。 |
| `transport` | `ws`、`http` | 外部入站传输方式。 |
| `account_id_key` | `bot_id` | 账号配置里用于生成 `account_id` 的字段。 |
| `receive_token_key` | `token` | 外部平台请求里 token 的 query key。 |
| `supports_qr_login` | `true` | 声明适配器提供扫码登录入口。它只影响设置页入口展示；真正发起扫码流程还要求当前 worker 在 `adapter_worker_ready.aut_params.supports_qr_login` 中同步声明运行时能力。 |
| `media` | `{"requires_file_path":true}` | 只有本地 SDK 必须读取文件路径时才声明。 |
...
- JSON Schema：[`schemas/plugin-manifest.schema.json`](https://autclaw.shop/cn/plugin-sdk/schemas/plugin-manifest.schema.json)
- JavaScript 类型声明：[`types/middleware-js.d.ts`](https://autclaw.shop/cn/plugin-sdk/types/middleware-js.d.ts)
