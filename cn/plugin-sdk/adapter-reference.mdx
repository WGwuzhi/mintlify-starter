...
| `supports_qr_login` | `adapter_login_qr_start`、`adapter_login_qr_wait` | 当前在线 worker 支持扫码登录或扫码绑定账号。 |
...
<Note>
  ...
  backend 真正发起 `adapter_login_qr_start`、`adapter_login_qr_wait` 前，只检查第二层运行时能力。
</Note>
