...
| 扫码登录 | 平台账号需要扫码授权或绑定 | 上述 core codec + `login_qr` + `adapter_login_qr_start`、`adapter_login_qr_wait` |
...
5. 如果适配器支持扫码登录，在 `adapter_worker_ready` 里上报 `login_qr: true`，并实现 `adapter_login_qr_start`、`adapter_login_qr_wait`。
...
<Info>
  `login_qr` 不是 manifest 头部字段。manifest 描述“这个插件是谁、参数是什么”；`adapter_worker_ready` 描述“这个运行中的 worker 现在能做什么”。
</Info>
...
| `adapter_worker_ready` | 声明当前 worker 支持的能力，例如核心 codec、worker 自发送、主动事件和可选的 `login_qr`。 |
| 业务 RPC | 处理账号解析、入站标准化、出站构造、回执解码和主动动作。 |

## 声明扫码登录能力

扫码登录属于运行能力，不写在插件头部注释里。只有适配器启动、连上 control websocket，并确认当前 worker 能处理扫码流程后，才在 `adapter_worker_ready` 中声明。
...
- 不支持扫码登录的适配器不要上报 `login_qr: true`。
- 已上报 `login_qr: true` 的适配器必须能响应 `adapter_login_qr_start` 和 `adapter_login_qr_wait`。
